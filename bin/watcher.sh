...
print_red "Watching folder: $AUTO_IMPORT/active files to import (*.osm, *.bz2, *.pbf)"
...
import_planet () {
  ...
  /usr/bin/osm2pgsql -c -U $POSTGRES_USER -d osm --slim $FILE
}
import_changeset () {
  FILE=$1
  print_cyan "**** importing changeset file: $FILE"
  /usr/bin/osm2pgsql -a -d osm -U $POSTGRES_USER --slim $FILE
}
process () {
  PATH=$1
  FILE_NAME=$2
